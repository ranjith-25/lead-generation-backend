...
async def get_user_status_by_id(db: AsyncSession, user_status_id: UUID):
    try:
...
async def update_user_status(db: AsyncSession, update_data: dict, user_status_id: UUID):
    try:
        result = await db.execute(
            select(UserStatus).where(
                UserStatus.id == user_status_id,
...
async def delete_user_status(db: AsyncSession, user_status_id: UUID):
    try:
