...
from app.services.db.user import get_all_user_by_role, get_all_manager
...
@router.get('/managers', response_model=list[UserRoleRead])
async def get_all_managers(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    return await get_all_manager(db)